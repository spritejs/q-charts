...
export class Axis extends BasePlugin {
  ...
  getDefaultAttrs() {
    const dObj = {
      ...
      coordPos: ['50%', '50%'],
      formatter: function(str, data) { // 格式化坐标轴文字显示
        return str
      }
    }
    return dObj;
  ...
  prepareAttrs(fields) { // 处理默认属性，部分属性会默认从对应visula继承过来
    ...
    if (this.chart.visuals.length >= 1 && $target === undefined) { // 如果visual大于等于一个
    ...
    if (axisGap === undefined && $target instanceof Bar && type === 'category') { // 特殊情况Bar特殊处理
  ...
  render(renderData = {}) {
    ...
    if ($target instanceof Pie && coord === 'polar') {
      ...
      rings = pieLayout()
        ...
        .endAngle(endAngle)
        .padAngle(padAngle)
        .value(d => +d[0].__valueGetter__())(this.getData().filter(d => !d[0].disabled));
    }
    return (
      <Group pos={pos}>
        <Group clipOverflow={false}>
          {renderData.scales.map((scale, i) => {
            let labelStyle = this.mergeTheme('label', [renderData.labelAttrs, scale, i]);
            let scaleStyle = this.mergeTheme('scale', [renderData.scaleAttrs, scale, i]);
            let gridStyle = this.mergeTheme('grid', [renderData.gridAttrs, scale, i]);
            let labelAnimation = {
              ...
              attrFormatter: (attr) => {
                if (typeof attr.text === 'number') {
                  return Object.assign(attr, { text: formatter(formatNumber(attr.text, scale.labelFrom.text, scale.labelTo.text)) });
                }
                return attr
              },
              useTween: true
            ...
            return (
              <Group size={[1, 1]} pos={scale.from.pos} clipOverflow={false}
                ...
                })}>
                {
                  labelStyle === false ? null : (
                    <Label {...labelStyle} clipOverflow={false} text={scale.labelTo.text} animation={this.resolveAnimation(labelAnimation)}></Label>
                  )
                }
                {
                  scaleStyle === false ? null : (
                    <Rect {...scaleStyle}/>
                  )
                }
                {
                  (coord === 'polar' || gridStyle === false || (scale.offset === 0 && !axisGap)) ? null : (
                    <Polyline {...gridStyle}/>
                  )
                }
              </Group>
        ...
        <Group size={size}>
          {renderData.scales.map((scale, i) => {
            ...
            return (
              (coord !== 'polar' || gridStyle === false || (scale.offset === 0 && !axisGap)) ? null : (
                <Circle pos={coordPos} radius={scale.offset} {...gridStyle} anchor={[0.5]}/>
              )
            )
          })}
        </Group>
        <Group clipOverflow={false}>
          {
            rings.map(ring => { // 绘制射线
              ...
              let newPoint = [pos[0] + maxRadius * Math.cos(angle), pos[1] + maxRadius * Math.sin(angle)]
              let labelPoint = [pos[0] + labelDis * Math.cos(angle), pos[1] + labelDis * Math.sin(angle)]
              return (
                <Group clipOverflow={false} size={['100%', '100%']}>
                  <Polyline points={[pos, newPoint]} color={'#dde0e5'} lineDash={[3, 4]}/>
                  <Label pos={labelPoint} color={'#67728C'} text={txt} fontSize={12} anchor={anchor}/>
                </Group>
              )
            })
          }
